...
func NewLoadTester(Link string, numRequests int64, concurrency int, timeout time.Duration, method string, headers map[string]string, proxies []string) *LoadTester {
	...
	if err := http2.ConfigureTransport(transport); err != nil {
		log.Printf("HTTP2 transport warning: %v", err)
	}
...
func (lt *LoadTester) sendRequest(ctx context.Context) {
	...
	lt.lastResponseCode = fmt.Sprintf("%d", code)
...
func (lt *LoadTester) run(ctx context.Context) {
	...
	sem := make(chan struct{}, lt.concurrency*2)
...
func animate(ctx context.Context, lt *LoadTester, initialCycleDuration, summaryDuration, updateInterval time.Duration) {
	...
	for {
		...
		case <-ticker.C:
			...
			pending := atomic.LoadInt64(&lt.numRequests) - atomic.LoadInt64(&lt.sentCount)
...
func main() {
	...
	updateIntervalFlag := flag.Float64("update-interval", 0.10, "ANIMASI")
	flag.Parse()
	...
	fmt.Printf("\n%sFinished: %d requests sent. Success: %d, Failed: %d%s\n",
		Hijau(""), done, atomic.LoadInt64(&lt.successCount), atomic.LoadInt64(&lt.failureCount), RESET)
}
